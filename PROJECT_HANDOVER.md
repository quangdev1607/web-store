# Project handover: Tiệm Bánh Bé Yêu

Tài liệu này dành cho người mới tiếp nhận dự án. Mục tiêu là đọc xong có thể tự chạy, tự lần code, tự hiểu luồng nghiệp vụ, biết sửa ở đâu và biết những chỗ nào đang là bẫy kỹ thuật.

Nếu tài liệu này khác `README.md`, `QNA_ANSWERED.md` hoặc `frontend/ARCHITECTURE.md`, ưu tiên tài liệu này trước, vì nó được tổng hợp lại theo code hiện tại trong repo.
...
Sau khi đọc hết danh sách này, người mới sẽ nắm được gần như toàn bộ luồng chính của dự án.

## 20. Tóm tắt một câu

Đây là một full-stack e-commerce demo khá đầy đủ cho cửa hàng bánh/kẹo trẻ em, đã có frontend, backend, DB, auth, admin, order và upload ảnh; nhưng trước khi dùng như sản phẩm thật cần sửa các gap về order authorization, cart sync, category slug, image/dev endpoint security, secret management và test coverage.
